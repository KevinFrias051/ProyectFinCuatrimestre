...
export function Filtro({ filtrarCandidatos, resetearFiltros }: any) {
  ...
  return (
    <div>
      <Select value={disponibilidad} onChange={handleDisponibilidadChange}>
        <MenuItem selected >Disponibilidad</MenuItem>
      ...
      <button onClick={() => filtrarCandidatos({ name: null, edad, disponibilidad, movilidad })}>Buscar
      </button>
      <button /* onClick={resetearFiltros(resetearFiltros)} */>
        Reset
      </button>
    </div>
  );
